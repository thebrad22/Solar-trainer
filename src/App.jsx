...
Respond ONLY with this exact JSON structure:
{
  ...
  const checkEnergyCommunity = async () => {
    const address = ecAddress.trim();
    if (!address || ecLoading) return;
    setEcLoading(true);
    setEcError(null);
    setEcResult(null);
    try {
      ...
      if (!response.ok || !data.matched) {
        setEcError(data.error || "Could not geocode that address.");
      } else {
        setEcResult(data);
      }
    } catch (e) {
      setEcError("Connection error reaching the geocoder — try again.");
    }
    setEcLoading(false);
  };
  ...
  return (
    <div style={{ fontFamily: "'Georgia', 'Times New Roman', serif", background: "#0f1923", minHeight: "100vh", color: "#e8dcc8" }}>
      ...
      <div style={{ background: "#111c11", borderBottom: "1px solid #2a3a2a", padding: "0 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", gap: 0 }}>
          {[
            ...
            { id: "energy", label: "🏭 Energy Community" },
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px" }}>
        ...
        {/* ===== ENERGY COMMUNITY TAB ===== */}
        {activeTab === "energy" && (
          <div>
            <SectionHeader icon="🏭" title="Energy Community Checker" />
            <p style={{ color: "#8aab8a", fontSize: 13, marginBottom: 20 }}>
              Look up the census tract for an address and see its 11-digit GEOID. Used to determine
              eligibility for the IRA Energy Community bonus credit (§§45, 45Y, 48, 48E).
            </p>
            ...
            <Card>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); checkEnergyCommunity(); } }}
                  placeholder="123 Main St, Albany, NY 12203"
                  style={{
                    flex: 1, minWidth: 220, background: "#111c11", border: "1px solid #2a4a2a", borderRadius: 8,
                    color: "#e8dcc8", padding: "10px 14px", fontSize: 14, fontFamily: "Georgia, serif", outline: "none",
                  }}
                />
                <button
                  onClick={checkEnergyCommunity}
                  disabled={ecLoading || !ecAddress.trim()}
                  style={{
                    background: ecLoading || !ecAddress.trim() ? "#1a2a1a" : "linear-gradient(135deg, #2a5c2a, #1a4a1a)",
                    border: "1px solid #4a8c4a", color: ecLoading || !ecAddress.trim() ? "#4a6a4a" : "#c8f0c8",
                    padding: "0 22px", borderRadius: 8, cursor: ecLoading || !ecAddress.trim() ? "not-allowed" : "pointer",
                    fontSize: 13, fontWeight: 700, letterSpacing: "0.05em",
                  }}
                >
                  {ecLoading ? "Checking..." : "Check Address"}
                </button>
              </div>
            </Card>

            {ecError && (
              <Card style={{ background: "#2a0d0d", border: "1px solid #5a2a2a", marginTop: 14 }}>
                <div style={{ color: "#ff9a9a", fontSize: 13 }}>⚠️ {ecError}</div>
              </Card>
            )}

            {ecResult && (
              <Card style={{ marginTop: 14 }}>
                <div style={{ fontWeight: 700, color: "#f0e68c", fontSize: 14, marginBottom: 10 }}>
                  📍 {ecResult.matchedAddress}
                </div>
                <div style={{ display: "grid", gap: 6, fontSize: 13, color: "#c8d8a8" }}>
                  <div><strong style={{ color: "#8aab8a" }}>Census Tract GEOID:</strong> <span style={{ fontFamily: "monospace" }}>{ecResult.geoid}</span></div>
                  <div><strong style={{ color: "#8aab8a" }}>Tract:</strong> {ecResult.tractName}</div>
                  <div><strong style={{ color: "#8aab8a" }}>County:</strong> {ecResult.county || "—"}</div>
                  <div><strong style={{ color: "#8aab8a" }}>State:</strong> {ecResult.state || "—"}</div>
                </div>

                <div style={{ marginTop: 16, background: "#1a1a0d", border: "1px solid #5a4a1a", borderRadius: 8, padding: 12 }}>
                  <div style={{ color: "#f0e68c", fontWeight: 700, fontSize: 13, marginBottom: 4 }}>
                    ⏳ Eligibility: Pending data integration
                  </div>
                  <div style={{ color: "#c8b89a", fontSize: 12, lineHeight: 1.5 }}>
                    This tool currently resolves the address to its census tract GEOID only. Matching this
                    GEOID against Treasury's Energy Community master table (Coal Closure and Statistical
                    Area / Fossil Fuel Employment categories from EC_AllTracts and the latest IRS Notice
                    appendices) has not been wired up yet. Look the GEOID up directly at{" "}
                    <span style={{ color: "#a8c888" }}>energycommunities.gov</span> until that integration ships.
                  </div>
                </div>
              </Card>
            )}

            <Card style={{ marginTop: 14, background: "#1a1a2d", border: "1px solid #3a3a5a" }}>
              <div style={{ color: "#a8a8f0", fontWeight: 700, fontSize: 13, marginBottom: 4 }}>
                🏗️ Brownfield sites — not covered by tract lookup
              </div>
              <div style={{ color: "#c8c8e8", fontSize: 12, lineHeight: 1.5 }}>
                Brownfield Category eligibility is site-specific (based on actual site contamination status
                or formal brownfield designation), not determined by census tract. A GEOID match here says
                nothing about Brownfield eligibility, and IRS Notice 2026-39 does not address the Brownfield
                Category at all. Verify Brownfield status separately for the specific parcel.
              </div>
            </Card>
          </div>
        )}
